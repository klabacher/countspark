...
export default function HeaderMenu() {
  ...
  const isForgotRoute = location.pathname === '/auth/forgot'
  const isResetStepsRoute = location.pathname === '/auth/steps'
  const isActivateRoute = location.pathname === '/auth/register/activate'
  const isSocialRoute = location.pathname === '/auth/social'
  ...
  return (
    <div className="sticky top-0 z-50 flex w-full items-center justify-between rounded-2xl border border-slate-200 bg-white/80 px-6 py-3 shadow-lg backdrop-blur-xl dark:border-white/10 dark:bg-white/5">
      ...
      {/* Toggle input checkbox - only show for login/register */}
      ...
      {/* Show title for other auth pages */}
      {(isForgotRoute || isResetStepsRoute) && (
      ...
      {isActivateRoute && (
        <div className="text-sm font-medium text-slate-500 dark:text-slate-400">
          {t('hud.AuthPage.Activate.headerTitle', 'Email Verification')}
        </div>
      )}

      {isSocialRoute && (
        <div className="text-sm font-medium text-slate-500 dark:text-slate-400">
          {t('hud.AuthPage.Social.headerTitle', 'Social Login')}
        </div>
      )}
